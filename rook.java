...
public class rook extends piece
{
  ...
  boolean move(board B)
  {
	...
	else if(picky == y && diffx > 0)
	  {
	    for(int i = x+1; i < pickx; i++)
	      {
		if(B.Grid().get(picky) != null)
		  tocheckrow = B.Grid().get(picky);
		if(tocheckrow.get(i) != null)
		  tocheck = tocheckrow.get(i);
		legalmove = true;
		if(tocheck.team != "unknown")
		  {
		    legalmove = false;
	...
	if(blocked == false && tocheck.team != team && legalmove == true)
	  {
	    if(team == "white" && tocheck.team == "black") /* white capture */                                                                                             
	      tocheck.alive = false;
	    if(team == "black" && tocheck.team == "white") /* black capture */                                                                                            
	      tocheck.alive = false;
